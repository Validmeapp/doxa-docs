---
title: "Autenticación"
description: "Aprende cómo autenticarte con nuestra API usando varios métodos"
version: "v1"
locale: "es"
order: 2
...
Nuestra API soporta múltiples métodos de autenticación para asegurar tus aplicaciones y proteger los datos de los usuarios.

## Claves de API

El método principal de autenticación utiliza claves de API que puedes generar en tu panel de desarrollador.

### Obtener Claves de API

1. Inicia sesión en tu panel de desarrollador
2. Navega a la sección "Claves de API"
...
1. **Solicitud de Autorización**: Redirige a los usuarios a nuestro servidor de autorización
2. **Consentimiento del Usuario**: Los usuarios otorgan permisos a tu aplicación
3. **Código de Autorización**: Recibe un código de autorización
4. **Intercambio de Token**: Intercambia el código por tokens de acceso
...
| `read:identity` | Leer datos de verificación de identidad |
| `write:identity` | Iniciar verificaciones de identidad |
| `read:webhooks` | Leer configuración de webhooks |
| `write:webhooks` | Gestionar webhooks |

## Tokens JWT

Para comunicación servidor-a-servidor, soportamos tokens JWT con firma RS256.
...
- Nunca expongas claves de API** en código del lado del cliente
...
Implementa manejo de errores apropiado para evitar filtración de información:
...
async function hacerSolicitudAPI(url, options) {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error('Error en la solicitud. Inténtalo de nuevo.');
    }
...
Implementa lógica de reintentos para manejar límites de velocidad:
  ...
  for (let intento = 0; intento < maxReintentos; intento++) {
    try {
      ...
      if (response.status === 429) {
        ...
        console.log(`Límite de velocidad alcanzado. Reintentando en ${delay}ms`);
...
| `429` | Límite de velocidad | Implementa lógica de reintentos |
...
¿Tienes preguntas sobre autenticación? Estamos aquí para ayudar:
...
¡Mantén tus integraciones seguras! 🔒